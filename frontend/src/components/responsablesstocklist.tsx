...
const StockManagerList: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      const usersRes = await api.getUsersByRole('StockManager');
      setUsers(usersRes);
    } catch (err) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10">
      <h2 className="text-4xl font-bold text-gray-800 mb-8 text-center">Gestionnaires de Stock</h2>
  ...
  >
    + StockManager
      ...
      {isLoading ? (
        ...
        <p className="text-gray-500 text-center text-lg">Aucun gestionnaire trouvé.</p>
      ) : (
          ...
          {users.map((user) => (
            ...
            >
              <div className="flex items-center gap-4">
                <img
                  src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user.role)}&background=0D8ABC&color=fff`}
...
>
  {user.etat === 'actif' ? '🟢 Actif' : '🔴 Suspendu'}
                  ...
                  {user.storageId && (
                    <p className="text-sm text-gray-400">ID Stockage : {user.storageId}</p>
                  )}
                </div>
              </div>
      ...
      {isEditModalOpen && selectedUser && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-40 flex items-center justify-center transition-all">
          <div className="bg-white rounded-xl shadow-xl p-6 w-[90%] max-w-md animate-fade-in">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Modifier l'utilisateur</h3>
            <input
              type="text"
      ...
      {isDeleteModalOpen && selectedUser && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-40 flex items-center justify-center transition-all">
          <div className="bg-white rounded-xl shadow-xl p-6 w-[90%] max-w-md animate-fade-in">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Supprimer l'utilisateur</h3>
...
{isAddModalOpen && (
  <div className="fixed inset-0 z-50 bg-black bg-opacity-40 flex items-center justify-center">
    <div className="bg-white rounded-xl shadow-xl p-6 w-[90%] max-w-md animate-fade-in">
      <h3 className="text-xl font-semibold text-gray-800 mb-4">Ajouter un responsable de stockage</h3>
      <input
        type="text"
        placeholder="Nom"
