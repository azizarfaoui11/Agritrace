...
const VendeurList: React.FC = () => {
  ...
  const fetchVendeurs = async () => {
    try {
      const usersRes = await api.getUsersByRole('Seller');
      setUsers(usersRes);
    } catch (err) {
 ...
 const handleEdit = (userId: string) => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10">
      <h2 className="text-4xl font-bold text-gray-800 mb-8 text-center">Vendeurs</h2>
  ...
  >
    + Vendeur
      ...
      {isLoading ? (
        ...
        <p className="text-gray-500 text-center text-lg">Aucun vendeur trouvé.</p>
      ) : (
...
{isAddModalOpen && (
  <div className="fixed inset-0 z-50 bg-black bg-opacity-40 flex items-center justify-center">
    <div className="bg-white rounded-xl shadow-xl p-6 w-[90%] max-w-md animate-fade-in">
      <h3 className="text-xl font-semibold text-gray-800 mb-4">Ajouter un vendeur</h3>
      <input
        type="text"
        placeholder="Nom"
